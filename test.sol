...
contract VoteElectroniqueUpgradeable is Initializable, AccessControlUpgradeable {
    ...
    function voter(uint _resolutionId, string calldata _voteType) external {
        ...
        if (voteHash == keccak256("pour")) {
            res.votesPour++;
        } else if (voteHash == keccak256("contre")) {
            res.votesContre++;
        } else if (voteHash == keccak256("neutre")) {
            res.votesNeutre++;
        } else {
    ...
    function obtenirResultats(uint _resolutionId) external view returns (uint8 pour, uint8 contre, uint8 neutre) {
        Resolution storage res = resolutions[_resolutionId];
